...
float SteeringAngleToRawAngle_rad(float steering_angle) {
	...
	float arm_wheel_angle_rad = NormalizePiToNegPi(radians(16.46f));		//16.46

	float servo_arm_to_arm_wheel_rod_length_mm = 45.6188202f;	// not necessary to calculate manually, already done automatically
	float arm_wheel_angle_position_rad = NormalizePiToNegPi((M_PI_2 * 3.0f)) - arm_wheel_angle_rad;
	float servo_rod_forward_angle_position_rad = NormalizePiToNegPi((M_PI_2 * 3.0f));
	float servo_rod_angle_position_rad = servo_rod_forward_angle_position_rad;
...
float RawAngleToSteeringAngle_rad(float raw_angle) {
	...
	float servo_arm_to_arm_wheel_rod_length_mm = 45.6188202f;	// not necessary to calculate manually, already done automatically
	float arm_wheel_forward_angle_position_rad = NormalizePiToNegPi((M_PI_2 * 3.0f)) - arm_wheel_angle_rad;
	float arm_wheel_angle_position_rad = arm_wheel_forward_angle_position_rad;
	float servo_rod_forward_angle_position_rad = NormalizePiToNegPi((M_PI_2 * 3.0f));
	float servo_rod_angle_position_rad = servo_rod_forward_angle_position_rad;
